...
function removeFirstOccurrences(str, value) {
  ...
  return index === Infinity ? '' : str.replace(value, '');
}
...
function removeLastOccurrences(str, value) {
  ...
  const end = str.slice(last).replace(value, '');
  return str.slice(0, last).concat(end);
}
...
function sumOfCodes(str) {
  let result = 0;
  if (str) {
    result = str.split('').reduce((summ, char) => {
      const newSum = summ + char.charCodeAt(0);
      return newSum;
    }, 0);
  }
  return result;
...
function formatTime(minutes, seconds) {
  const min = ''.padStart(minutes < 10 ? 1 : 0, '0') + minutes;
  const sec = `${seconds < 9 ? '0' : ''}${seconds}`;
  return `${min}:${sec}`;
}
...
function isPalindrome(str) {
  const lower = str.toLowerCase().replace(/[^A-Za-z]/g, '');
  const lowerReverse = lower.split('').reverse().join('');
  const start = [];
  const end = [];
  const length = Math.trunc(str.length / 2);

  for (let i = 0; i <= length; i += 1) {
    start.push(lower[i]);
    end.push(lowerReverse[i]);
  }
